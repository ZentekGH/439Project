...
# global to hold the path specs currently being tracked
...
Vmax_YAML = rospy.get_param('/Vmax')
Vmax = Vmax_YAML
...
# halves max v if we're over a dark surface
def modify_max_v(msg_in):
    global Vmax
    if (msg_in.data > 0):
        Vmax = 0.5 * Vmax_YAML
    else:
...
####    CODE HERE:  Put in formulas for anything that is 0.0, and try the "TRY THIS" variations. 
    ...
    # set the center speed of the robot based on Vmax
    ...
    if forward_only: 
        Vc = np.max([Vc,0])  # equivalently: Vc = np.max([Vc,Vc*int(forward_only)])
